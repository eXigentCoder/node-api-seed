...
function ensureExactly1Key(object, paths, checkType) {
    ...
    paths.forEach(function(path) {
        ...
        if (found) {
            throw new Error(
                util.format(
                    'Object should only have one of the following paths being truthy : %j%sThe object was : %j',
                    paths,
                    eol,
                    object
                )
    ...
    if (!found) {
        throw new Error(
            util.format(
                'Object should have exactly one of the following paths being truthy : %j%sThe object had none and was : %j',
                paths,
                eol,
                object
            )
...
function ensureAllKeys(object, paths, checkType) {
    ...
    paths.forEach(function(path) {
        ...
        if (!valueValid(value, checkType)) {
            throw new Error(
                util.format(
                    'Object should only have all of the following paths being truthy : %j%sThe object was : %j',
                    paths,
                    eol,
                    object
                )
